...
**Mechanism established 2026-07-05 — the Split Brain and the grid catastrophe are very likely the same event, not two separate consequences of the war.** Amundsen Tower's reframed Primary Purpose (see `Theoretical-Calculations/Amundsen_Tower_Space_Fountain_Design.md`) makes it the continent's energy-regulating flywheel — meaning the Arcanet's inter-subnet backbone almost certainly depended on that same Tower-regulated grid to function at all. When the Tower collapsed, the resulting continent-wide power overload (see `Energy_Grid_Failure_Rationale.md` #11, "the Amundsen Resonance Effect," for the full electrical mechanism — a real-world "load rejection" event) plausibly fried or severed the inter-subnet Arcanet links specifically, while each subnet's *local*, self-contained network — not dependent on the same continent-spanning backbone — survived intact. This is why the Split Brain cut cleanly at subnet boundaries rather than causing uniform, generalized data loss everywhere: the failure mode was electrical and structural, not random or purely military.

**~15 years of independently-developed, potentially conflicting history per subnet.** Since the war (2812) to the game's present day (≈2822–2827), each subnet has had roughly a decade to a decade and a half with zero cross-subnet information exchange — no way to verify, correct, or reconcile the other five subnets' records against its own. This is the concrete timescale behind "developing its own version of historical records" above: not a vague or symbolic gap, but a specific ~15-year window in which six mutually isolated information environments could each settle into their own internally-consistent (and mutually inconsistent) account of what happened, who's responsible, what the casualty figures were, and how the war and its aftermath are remembered.
