# Casey — Neo-Race Cataloging (Phase 1)

**Status:** City Snapshot, Real-World Parallel Locations, City-Type Parallels, and Population Weighting
filled 2026-07-16. **Per-Nation Entries (Phase 1c) complete 2026-07-16** — first Mirny subnet city
finished. Drafted against `City_Enneagram_Personalities/Mirny_Subnet/Casey.md` and
`Casey_Mega_Init.md`'s Inspirational Influences research from the start, per the standing process
requirement. Synthesis Notes (Phase 2) drafted below as a working first pass, not yet
developer-confirmed as final.

---

## City Snapshot

- **Nations present, by tier:** Primary — China, USA. Significant — Japan, France, South Korea,
  Russia, Indonesia, Australia (founding wave — shortest Australia-to-Antarctica route of any Tepenian
  city). Notable — Thailand, Vietnam, Philippines, Malaysia, New Zealand. Exact per-nation share % in
  `Specs/Casey.md` and `Official_Population_Census.md`.
- **Geography:** Budd Coast, Wilkes Land, Vincennes Bay — sits almost exactly on the Antarctic Circle
  threshold (66°16'S), giving it minimal but real polar night/midnight sun. Wilkes Station ruins (a
  genuine pre-exile artifact) stand nearby.
- **Geological composition:** Sheltered coastal inlet, moderate katabatic influence funneling from the
  plateau — milder than the more extreme Janbogo subnet coastal cities.

---

## Population Weighting Reference (for future Phase 1c cultural weighting)

*Source: `Specs/Casey.md`, Per-Nation Breakdown table (all tiers, not just Primary/Significant). Recorded here so cultural findings for this city can later be weighted by actual population share rather than treated as if every listed nation contributed equally.*

| Tier | Nation | Share % |
|------|--------|---------|
| Primary | USA | 21.96% |
| Primary | China | 19.06% |
| Significant | South Korea | 10.46% |
| Significant | France | 10.05% |
| Significant | Australia | 9.26% |
| Significant | Russia | 6.13% |
| Significant | Japan | 5.33% |
| Significant | Indonesia | 4.92% |
| Notable | New Zealand | 3.36% |
| Notable | Philippines | 3.35% |
| Notable | Vietnam | 3.21% |
| Notable | Malaysia | 1.78% |
| Notable | Thailand | 1.13% |

*(Total: 100.00%)*

---

## Real-World Parallel Locations (Phase 1b — location identification only, no cultural findings yet)

**Terrain profile:** sheltered coastal inlet with moderate katabatic influence — milder than the
extreme-wind cities elsewhere in the country, closer to a genuinely liveable sheltered-bay profile.

- **China (Primary):** Qingdao's sheltered bay coastline, Shandong — a real, moderate sheltered
  Chinese coastal city.
- **USA (Primary):** Sitka, Alaska — a genuine sheltered coastal inlet town with moderate
  mountain-influenced wind, a strong match.
- **Japan (Significant):** Wakasa Bay, Fukui — a real sheltered Japanese bay with moderate seasonal
  wind.
- **France (Significant):** Gulf of Morbihan, Brittany — a real sheltered French coastal bay.
- **South Korea (Significant):** Jinhae Bay — a real sheltered Korean bay with moderate
  mountain-funneled wind.
- **Russia (Significant):** Avacha Bay, Kamchatka (near Petropavlovsk-Kamchatsky) — a genuine
  sheltered bay with nearby volcanic mountains producing real wind funneling, a strong match.
- **Indonesia (Significant):** No strong match — tropical Indonesia lacks a comparable moderate
  katabatic-bay profile; flagged as weak.
- **Australia (Significant):** Hobart / the Derwent estuary, Tasmania — a real sheltered
  hill-protected Australian bay; a fitting match given Casey's own direct founding wave from
  Australia.

---

## City-Type Parallels (Phase 1b-ii — functional/civic identity, not geography)

**City type:** highway junction/transit hub, PLUS the project's own second worked example for this
whole category: "Splinters," Tepenia's largest and most famous bar (Classic Rock/Jazz/Blues/Acoustic
Folk music scene), famous along the whole Dumont d'Urville Sea and Indian Ocean coastline.

- **China (Primary):** Chengdu's teahouse-and-live-music culture — a genuine, nationally-known Chinese
  leisure/entertainment-district match.
- **USA (Primary):** Nashville's Broadway strip / the Grand Ole Opry — a nationally (and
  internationally) famous live-music destination; an excellent, direct match.
- **Japan (Significant):** Osaka's Dōtonbori — a real, nationally famous Japanese entertainment
  district.
- **France (Significant):** Paris's Pigalle/Moulin Rouge district — a nationally (and
  internationally) famous French entertainment-leisure landmark.
- **South Korea (Significant):** Hongdae, Seoul — a real, nationally famous Korean nightlife/live-music
  district.
- **Russia (Significant):** Moscow's Tverskaya nightlife strip — a moderate domestic match, weaker
  than the others; flagged for Phase 2 review.
- **Indonesia (Significant):** No single comparably famous national leisure landmark — flagged as
  weak.
- **Australia (Significant):** Kings Cross, Sydney — historically Australia's most famous
  entertainment/nightlife district; an especially fitting match given Casey's own direct founding wave
  from Australia.

---

## Per-Nation Entries (Cultural Iceberg findings — Phase 1c, complete 2026-07-16)

**No National Stereotypes compliance note:** every finding below is framed as arising from Casey's own
conditions (its highway-junction civic role and its Splinters-anchored entertainment identity) that a
given population's originating patterns fed into or reinforced — never as "this population does X
because they're from nation Y."

**Pre-existing research folded in:** Casey's Enneagram profile is **Feeling (Heart) / Assertive /
Positive Outlook** — the same three traits as Palmer City, a city organized around felt experience and
projecting its own cultural identity outward. `Casey_Mega_Init.md`'s own Inspirational Influences
research already identifies Casey's *actual* established founding tension, directly from its own
Post-Culture Identity section: **destination versus waypoint** — "residents live full lives in a city
whose external reputation reduces it to a highway junction." Three real-world precedents ground this,
all tied to the USA population specifically: Flagstaff, Arizona (a literal crossroads by both history
and design, civic identity and junction function treated as the same fact); Memphis, Tennessee (a
multi-modal crossroads where Beale Street's live-music scene sits directly at the junction and became
the city's own defining civic landmark — proof of exactly Casey's own Splinters dynamic); and Reno,
Nevada (the closest real precedent for the destination-vs-waypoint tension itself — a real 1931 pivot
from stopover to destination via legalized gambling, not just an invented parallel).

### USA (Primary, 21.96%)

**Real-world parallel community/communities identified:** Sitka, Alaska (geography — a strong sheltered
coastal match) and, on the City-Type side, **three** distinct precedents rather than one: Flagstaff
(the crossroads-identity match), Memphis/Beale Street (the Splinters-precedent match), and Reno (the
destination-vs-waypoint tension match). This population is the most richly, precisely anchored found
anywhere in this project so far on the City-Type dimension alone.

#### Surface Culture findings
- **Performances, Music:** Beale-Street-style live-music-as-civic-landmark culture, directly informing
  Splinters' own established Classic Rock/Jazz/Blues/Acoustic Folk scene.
- **Language:** crossroads-identity self-description, echoing Flagstaff's own real civic
  self-characterization as "the great crossroads."

#### Deep Culture findings

**Communication styles and rules:** direct and practical, with a genuine, well-practiced skill at
quickly sizing up strangers — is this person just passing through, or here to stay? Locals develop real
social fluency at reading that distinction fast, a skill crossroads-town life demands.

**Notions of:** courtesy built on hospitality-to-travelers, echoing Flagstaff's own real
crossroads-service economy; local pride carried privately rather than performed publicly — residents
wear Splinters' regional fame with real satisfaction while publicly downplaying Casey as "just a
junction," a genuine, lived modesty rather than false humility.

**Concepts of:** self split cleanly into two coexisting layers — the full, rich internal civic life
residents actually lead, and the flattened external reputation ("mere highway junction") outsiders
know Casey by. Most residents hold both without contradiction, the same duality Reno's own real 1931
stopover-to-destination pivot demonstrates historically. Time organized around who's passing through
this week or month as much as by any fixed calendar.

**Attitudes toward:** newcomers and strangers as a constant, genuinely welcomed presence rather than a
novelty or disruption — a structural fact of daily life, not an occasional event.

**Approaches to:** identity-assertion handled through insider knowledge rather than public argument —
knowing Casey is more than its reputation is itself a quiet form of belonging, not something residents
feel a need to prove to outsiders.

---

### China (Primary, 19.06%)

**Real-world parallel community/communities identified:** Qingdao's sheltered bay coastline (geography
— a strong match) and Chengdu's teahouse-and-live-music culture (civic type — a genuine, nationally
known Chinese leisure/entertainment-district match).

#### Surface Culture findings
- **Food, Music:** Chengdu's own real teahouse-and-live-music tradition — communal, unhurried, a
  genuine convergence point with Splinters' own live-music identity via a distinct national register.

#### Deep Culture findings

**Communication styles and rules:** relaxed and unhurried, echoing Chengdu's own real reputation as
China's most laid-back major city — long, low-stakes social conversation valued over quick
transactional exchange, a real contrast with the junction city's transit-driven public rhythm.

**Notions of:** leisure itself as legitimate, valued time — not merely a reward earned after work, but
worth protecting and prioritizing on its own terms.

**Concepts of:** time as something to be spent generously in company, genuinely in tension with (not
simply replaced by) Casey's own transit-driven civic pace — a real, felt push-pull rather than a
resolved contradiction.

**Attitudes toward:** music and performance as communal and participatory, echoing teahouse tradition,
rather than a purely spectator experience the way a formal concert hall would be.

**Approaches to:** socializing as a regularly-practiced art form, not something reserved for special
occasions.

---

### South Korea (Significant, 10.46%)

**Real-world parallel community/communities identified:** Jinhae Bay (geography — a real sheltered
Korean bay with moderate mountain-funneled wind) and Hongdae, Seoul (civic type — a real, nationally
famous Korean nightlife/live-music district).

#### Surface Culture findings
- **Music, Fashion, Performances:** Hongdae's own real live-music/street-fashion/youth-culture
  identity, a strong, direct convergence with Splinters.

#### Deep Culture findings

**Communication styles and rules:** expressive and visually attentive, echoing Hongdae's own real
association with street fashion and current trends — self-presentation carries real communicative
weight here.

**Notions of:** cultural currency and relevance tied to nightlife-district participation — being
genuinely "in" on the current scene is a real, valued form of social capital.

**Concepts of:** self-presentation and belonging bound up together — showing up, being seen, and
knowing what's current are all the same underlying social skill.

**Attitudes toward:** creative and artistic subculture as something to actively join and contribute to,
not simply observe from the outside.

**Approaches to:** socializing organized around discovery — actively following and finding what's new,
rather than settling into fixed habitual venues.

---

### France (Significant, 10.05%)

**Real-world parallel community/communities identified:** the Gulf of Morbihan, Brittany (geography —
a real sheltered French coastal bay) and Paris's Pigalle/Moulin Rouge district (civic type — a
nationally, internationally famous entertainment-leisure landmark).

#### Surface Culture findings
- **Performances, Fashion:** Pigalle's own real cabaret/entertainment-district heritage, a strong
  convergence with Splinters' own civic-landmark status.

#### Deep Culture findings

**Communication styles and rules:** a theatrical, performance-inflected register even in everyday
exchange, echoing Pigalle's own cabaret heritage — self-presentation carries a deliberate, stylized
quality.

**Notions of:** beauty and spectacle as a public, communal good, meant to be shared and witnessed
together rather than experienced privately.

**Concepts of:** nightlife fame treated as a genuine civic art form, worth deliberate cultivation and
craft, not incidental to the city's identity.

**Attitudes toward:** performance and showmanship as skilled labor deserving real respect and status,
not dismissed as frivolous.

**Approaches to:** courtship and social life frequently organized around shared entertainment venues —
the venue itself as a social institution, not just a backdrop.

---

### Australia (Significant, 9.26%, founding wave)

**Real-world parallel community/communities identified:** Hobart/the Derwent estuary, Tasmania
(geography — a real sheltered hill-protected bay) and Kings Cross, Sydney (civic type — historically
Australia's most famous entertainment/nightlife district), both especially fitting given Casey's own
direct founding wave from Australia.

#### Surface Culture findings
- **Performances, Music:** Kings Cross's own real nightlife-district heritage, doubly resonant given
  the founding-wave connection.

#### Deep Culture findings

**Communication styles and rules:** informal, easygoing, self-deprecating humor as a default register —
consistent with the founding-population's own long-settled comfort in the city.

**Notions of:** founding-population authority — being able to say "we were here first" carries real,
if understated, social weight rather than being loudly asserted.

**Concepts of:** self bound up with origin-story ownership — a quiet but genuine founding-wave pride,
different in register from Argentina's more overt historical claim at Esperanza but comparable in kind.

**Attitudes toward:** newcomers treated warmly and without suspicion, consistent with Australia's own
role here as the arriving, welcomed founding population rather than a defensive incumbent one.

**Approaches to:** nightlife culture treated as inherited civic legacy, passed down through the
founding generation rather than something each new arrival discovers independently.

---

### Russia (Significant, 6.13%)

**Real-world parallel community/communities identified:** Avacha Bay, Kamchatka (geography — a strong
match, a genuine sheltered bay with volcanic wind-funneling and a real remote, weather-hardened
community) — Moscow's Tverskaya nightlife strip (civic type — a moderate, notably weaker domestic
match than the others).

#### Surface Culture findings
- **Food, Music:** a modest nightlife contribution, weaker than the other populations' own City-Type
  anchors, with more genuine texture coming from the Avacha Bay geography match instead.

#### Deep Culture findings

**Communication styles and rules:** reserved in public settings, warmer and more open in small,
familiar gatherings — echoing Avacha Bay's own real close-knit, practical, weather-hardened community
character rather than any generic assumption.

**Notions of:** hospitality valued but not showy — practical warmth expressed through action (shelter,
provision) rather than public display.

**Concepts of:** place and belonging shaped by physical shelter itself — a "protected pocket" self-
concept, echoing the bay's own real function as a refuge from open, harsher water.

**Attitudes toward:** nightlife and public entertainment as a secondary, modest pursuit rather than a
defining identity, consistent with this population's weaker City-Type anchor here.

**Approaches to:** socializing conducted in smaller, closer circles rather than large public venues.

---

### Japan (Significant, 5.33%)

**Real-world parallel community/communities identified:** Wakasa Bay, Fukui (geography — a real
sheltered Japanese bay, a quiet fishing-village register) and Osaka's Dōtonbori (civic type — a real,
nationally famous, visually loud entertainment district) — genuinely different registers within this
one population's own real-world matches.

#### Surface Culture findings
- **Performances, Food:** Dōtonbori's own real food-and-entertainment fusion identity (its famous
  illuminated signage and street-food culture inseparable from each other), a strong convergence with
  Splinters.

#### Deep Culture findings

**Communication styles and rules:** a genuine public/private duality, mirroring the two source
matches directly — an expressive, visually oriented persona in entertainment-district contexts
(Dōtonbori), a quieter, more reserved register in ordinary daily life (Wakasa Bay).

**Notions of:** spectacle and visual display as a legitimate, celebrated public form — Dōtonbori's own
iconic signage culture treats being seen as a genuine civic art, not vanity.

**Concepts of:** self as context-dependent — which register (public spectacle vs. private reserve)
applies depends on setting, held as a normal, unremarkable fact of social life rather than a tension.

**Attitudes toward:** food and entertainment treated as inseparable from each other, not two separate
categories of leisure.

**Approaches to:** socializing organized around specific entertainment-district visits as a defined
event, not a diffuse, ongoing backdrop.

---

### Indonesia (Significant, 4.92%, smallest population)

**Real-world parallel community/communities identified:** no strong match on either geography or
City-Type — flagged as weak, consistent with the established pattern for this nation at narrower, more
specific City-Type categories.

#### Surface Culture findings
- **Food, Music:** general Indonesian cultural baseline persists — communal food-sharing, warm
  hospitality — without a strong local-specific anchor to sharpen it further.

#### Deep Culture findings

**Communication styles and rules:** warm and relational as a general diaspora baseline, though without
a strong local anchor to give it Casey-specific texture the way the better-matched populations have.

**Notions of:** community and collective harmony valued as a general cultural throughline, present but
genuinely thinner here than at cities where this population holds a strong City-Type match.

*(Honestly thin beyond these two fields — no strong anchor found on either dimension for this
population at this city, and not padded out further.)*

---

## Synthesis Notes (Phase 2 — draft, not yet developer-confirmed as final)

**Working name: "Caseyite"** *(placeholder, same status as every other city's working name so far).*

Casey's neo-culture centers on a genuine internal/external identity split — the **destination-versus-
waypoint** tension named directly in the city's own established lore, not an invented Phase 2 reading.
Every population here contributes to Splinters' shared entertainment-district identity (Chengdu,
Hongdae, Pigalle, Kings Cross, Dōtonbori all converge on live music/nightlife as civic pride), but USA
carries disproportionate narrative weight: it's the only population with *three* distinct, precisely
matched real-world precedents (Flagstaff for the crossroads identity itself, Memphis/Beale Street for
the Splinters mechanism specifically, and Reno for the destination-vs-waypoint tension), each mapping
onto a different facet of Casey's own established civic character rather than one generic reuse.

Australia's founding-wave status gives it a genuine, quieter claim to Casey's own origin story,
comparable in kind (though gentler in register) to Argentina's louder historical claim at Esperanza.
Russia and Japan both show internal splits worth remembering: Russia's geography match (Avacha Bay)
carries more real texture than its weak City-Type match; Japan's *two* matches point to genuinely
different registers (public spectacle vs. private reserve) rather than one unified reading. Indonesia
remains the one honestly thin population, consistent with the established pattern for narrow City-Type
categories.
