...
- Convert Cyberpunk 2077's skill-based bonuses into "challenge perks," and also into ordinary level-up perks where that fits better — source material is CP2077's own skill trees and rank-up bonuses, both pre-2.0 (1.63-era, developer's preferred system) and post-2.0. Per direct developer instruction: this is explicitly a mixed sort, not a 1:1 conversion, and per a later instruction, nothing rejected gets deleted — see `feedback_conversion_material_retention_policy` memory. **Progress, 2026-08-02:** source material complete (`Reference/cp2077-1.63-skill-level-rewards.txt`, `cp2077-1.63-perk-trees.txt`, `cp2077-2.0-skill-rank-bonuses.txt`, `cp2077-2.0-perk-trees.txt` incl. Relic tier); methodology doc complete (`Game-Mechanics/Perks/CP2077_Conversion_Basis_of_Translation.md`); free-flow drafting **complete, 2026-08-02** — all 7 planned batches drafted and merged into `to-be-integrated/exterior-game-imports/cyberpunk_2077.txt` (1.63 skill-level rewards, 1.63 perk trees across all 12 skills, 2.0 role-skill rank bonuses, 2.0 perk trees across all 5 Attributes including the 9-perk Relic tier). Matches the equivalent BG3 pass (`baldurs_gate_3.txt`, all 309 triaged entries), also fully drafted. **Next step for both:** this is still free-flow/illustrative material — no final numbers, no developer-approved names. Several open design questions surfaced along the way and remain genuinely unresolved: RAM-equivalent hacking resource, whether cyberware capacity applies to Inner Tepenia's robot player characters at all, no item-rarity/quality-tier system, and (newly surfaced) no Finisher/execute mechanic exists yet. Once reviewed, promising entries move into `DRAFT_Perks.txt` / `DRAFT_Traits.txt` and eventually the real destination docs (`Regular_Perks_-_Level-Up.md`, a quickhack reference, `Traits.md`, etc.).
