...
**Roster source of truth — updated 2026-07-20 with the developer's precise, final pass on recruitability status.** Every Doll character folder under `Worldspace/Characters/Dolls/Still-Present_-_In-Game/` falls into one of four categories:

- **Confirmed non-recruitable, permanently:** **Trisha Miller** (her whole post-questline payoff — world-state feedback delivered through her radio show — depends specifically on her never leaving that role) and **Majyao Bisyugota** (already has a fully-designed romanceable-without-recruitable arc — repeated teahouse visits, stat gates, the Blood River Tea thread — that depends on her staying at the teahouse).
- **Structurally distinct special case, not a normal recruitability question at all:** **Calethina** — holographic projection, no party slot, no physical dwelling, exempt from Companion Slot Rules, already fully romanceable through her own unique download/projection system; "almost automatically" a companion in an unorthodox way the normal recruitment flow doesn't apply to.
- **Undecided — genuinely not settled, may end up recruitable or may follow the Majyao pattern instead (full companion + romance questline, never joins the active party):**
  - **FR-03 "Maria"** — developer's own instinct: she may end up operating a leisure establishment (a billiards hall, matching her working name), which would favor the Majyao-style stay-in-place pattern.
  - **SE-031 "Akina"** — the gynoid at the center of the Long Night War's actual inciting incident (see `TODO.md` and the project's own `[[project_long_night_war_inciting_incident]]` context); her story is explicitly flagged by the developer as extremely delicate and requiring the utmost care, which is part of why her recruitability specifically hasn't been settled yet.
  - **TCY-20 "Miranda"** — developer's own instinct: she may end up operating an actual in-game bar as its bartender, which would similarly favor the stay-in-place pattern.
  
  All three would, if resolved non-recruitable, get the same treatment as Majyao: a full companion questline and a full romance arc, just anchored to a fixed location rather than joining the party. **Do not design their MACHINE-stat-gated romance thresholds or companion perks assuming party membership until this is resolved** — the Majyao-style pattern changes how their content should be structured (location-anchored repeat visits, not travel-with-the-player beats).
- **Everyone else in that folder — including every still-TBN-named placeholder character — is confirmed, not just plausible, as a future recruitable and romanceable companion.** This resolves the ambiguity on "Charlene" (XT-17, DLC 7), previously listed as "presumptive only, Undecided," and on **XT-21 "Angelina"**, whose own file still says "Companion Potential: Undecided" — both are now confirmed recruitable; their own files just haven't caught up yet (see "Practical consequence" below).

As of 2026-07-20, the folder contains 31 entries (1 of which, `z-template`, is a blank template, not a character). Excluding the 2 permanently non-recruitable NPCs, Calethina's special case, and the 3 genuinely undecided characters, that leaves **25 confirmed recruitable companions** existing in some form already (fully designed, partially designed, or bare TBN placeholder) — with Maria, Akina, and Miranda as a possible 3 more once resolved — before any of the new DLC-tier companions from the "Multiple Native Companions Per DLC" policy are added on top. This list will grow as new characters are created — the rule is about the folder as a whole, not a fixed enumeration.
