...
**⛔ Before any cold run: check `00_RUNBOOK.md` `Step −2`'s leak register.** Runs 12 and 13 were both burned.
→ record: `Test_Runs/RUN_LOG.md` · findings: `Test_Runs/OBSERVATIONS_and_Methodology_Findings.md` M-87–M-109

### ⏸️ Needs a developer ruling
- **Exposure ledger** (M-89) — only check that catches a compositional leak; per-run cost
- **`01_Frame_Typology_and_Inheritance.md` line 65** — genericize Casey's `Resettled` mention, or leave skipped
- **A deliberate thin-Settlement run?** — 32 of 37 cities are stub-tier (M-100)
- **`CGRM-001` Denison geology** — parked → `Canon_Gap_Resolution_Method/Gap_Registry.md`
