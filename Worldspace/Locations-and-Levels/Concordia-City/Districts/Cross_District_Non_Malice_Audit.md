# Cross-District Non-Malice Audit
...
## RESOLVED — confirmed passing or reworked, edits already made to `District_Canon_Reference.md`

### Gemini — "The Great Corruption" — confirmed passing, locked
Left as genuinely ambiguous (accidental vs. deliberate, in-world mystery) but now explicitly locked: if this ever gets resolved, it must resolve toward an accident (inadequate founding-era storage/redundancy, hardware failure or crisis-era mishandling that looks suspicious in hindsight) — never toward actual deliberate sabotage.

### Taurus — "The Insulation Schism" — confirmed passing as-is
Patronage-flavored emergent inequality (connections determined who got scarce insulation during shortages) — structurally the same shape as Capricorn's own confirmed-viable "guild patronage over merit" mechanism. Nobody is targeting the cold-side families; no rework needed.

### Virgo — "The Year of Silence" — confirmed passing as-is
A one-time, reactive, justified response to being systematically overlooked despite equal wartime sacrifice (working strictly to contract until the city's neglect became impossible to ignore) — not exploitation, a boundary asserted once under real grievance, never repeated. Different moral register from the two still-unresolved violations below.
...
## Passes cleanly — no action needed

- **Cancer:** Mother's Circuit (began benevolently, drifted into surveillance without anyone intending it), the Overcrowding Decision (crisis triage, not malice).
- **Taurus:** Bonded Pair Registry Collapse (pure bureaucratic accident).
- **Aquarius:** its entire identity is already explicitly written as the approved pattern — *"Aquarius is not malicious. It is something potentially more dangerous: certain."* A model example worth referencing when reworking other districts' material.
- **Libra:** the Suspended Compact (emergency powers never formally returned — classic structural inertia), the Perpetual Emergency (deliberately ambiguous whether Libra cultivates or merely benefits from it — leans passive as written).
- **Gemini:** the Twin Channels (non-malicious structural duality), the Speed Problem (a cultural value that never updated).
- **Pisces:** The Flood (pure accident), the Tolerance Pact (mutually beneficial, consensual — though already separately flagged as uncertain for unrelated reasons, same as the Star War), the Long Night War black-market lifeline (community-serving, no malice).
- **Sagittarius:** the Third Expansion Collapse (structural miscalculation + weather), the Siligel Rush and Bust (pure boom-bust economics), chronic neglect from Libra's central planning (systemic, not targeted).
- **Virgo:** the general recognition/attention gap with Aries (emergent, nobody choosing to slight Virgo) — separate from the Year of Silence above, which is its own confirmed-passing entry.
- **Hub:** nothing in this district's material involves active malice.

---

## Cross-District Notes entry, flagged for the same reason, not yet actioned

`District_Canon_Reference.md`'s own "Cross-District Notes" section closes with: *"The districts nobody officially acknowledges needing are Pisces and Scorpio. Both of them know this. Both of them use it."* "Use it" implies some form of active leverage, in the same family as the Aries/Libra items above — vague enough that it may not need independent action, but worth keeping in mind if Pisces' or Scorpio's own material gets reworked.
