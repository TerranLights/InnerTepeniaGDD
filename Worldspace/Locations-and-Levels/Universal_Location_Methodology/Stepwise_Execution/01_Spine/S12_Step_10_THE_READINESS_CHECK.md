...
> **Origin: `/home/kuroskalacs/Documents/Doll-Fi/media/games/Inner Tepenia/InnerTepeniaGDD/Worldspace/Locations-and-Levels/Universal_Location_Methodology/00_RUNBOOK.md` — lines 3001–3091.** *(Range refreshed 2026-09-14 — re-verified against the current file, TEXT re-synced against the same sweep; was `2860–2975`, itself stale since a 2026-09-07 "refresh" that moved only the declared number and never re-checked the body. Body text below was checked word-for-word against the current runbook and needed no change — this card's DRIFT was pointer-only, unlike most of its siblings; see `R-10`.) ⚠ **UPPER BOUND CORRECTED FROM AN INITIAL `3116` (end of file) TO `3091`** — Step 10 is the LAST step, so the sweep script's own boundary rule ("next `# Step` heading, or EOF") wrongly used EOF and swept in the trailing `# Where everything lives` reference table, which is project-index material, not Step 10 content. The card's own body was never wrong — it already stopped at the correct point; only this declared range briefly overstated it. Caught by a content-coverage check run immediately after the fix, not by the original sweep.* ***If this file and the source ever disagree, THE SOURCE WINS.***
