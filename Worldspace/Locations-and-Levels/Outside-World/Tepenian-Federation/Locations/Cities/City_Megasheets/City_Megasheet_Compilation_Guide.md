...
6. **Only now, research the real-world Inspirational-Influences picks.** For each Primary/Secondary entry, run an actual web search on that specific real place — its architecture, history, cultural role, whatever's documented — and then ask, for each one individually: *what does this specific real place's own documented character give back to the Tepenian city that isn't already there?* This is the step most likely to be done badly if rushed: the failure mode is treating the research as decorative trivia ("here's a fun fact about Angkor Wat") rather than as material to actively fuse ("Angkor Wat's cosmology, inverted, explains something about Dome Fuji's own established flatness that nothing else in the existing lore explained"). Every research finding that makes it into the Mega-Init should be answering that fusion question, not just reporting what was found.
   - **Standing rule, added 2026-07-07, sharpened same day: which real-world nation founded a Tepenian city's own underlying Antarctic station is irrelevant to this whole process, full stop.** That fact exists purely for GPS/geographic purposes — locating where something is on the real map — and carries no thematic weight of its own. Never reach for a real-world inspiration city, or evaluate whether one "fits," on the basis of national matching (e.g., picking an Italian city because a Tepenian city's founding station happened to be Italian, or ruling one out because it isn't). This applies even when a founding nation is already established, load-bearing canon for the Tepenian city itself (as with Zukelli's own genuinely Italian civic identity) — that fact can be discussed on its own terms, but it is never a reason to prefer or reject a specific real-world research pick. **Sharpened, same day:** don't even *note* national alignment or mismatch, in either direction — no "none of these three picks share the founding nation" disclaimer, no reassurance that a check came back clean. Tracking the question at all, even defensively, is the wrong mental motion; the founding nation should simply never enter the reasoning. Every research finding's justification should rest entirely on structural or thematic fusion, full stop, with no accompanying commentary about national alignment either way.
...
4. **Correction, 2026-07-07 — the Cross-Reference Synthesis stays purely descriptive.** Within *this* document, 2nd-, 3rd-, and 4th-order effects are just successive links in a chain of implication — each one stated plainly, in the same descriptive voice, with no design-role labeling attached. Do **not** frame a 3rd-order effect as "a questline seed" or a 4th-order effect as "a dialogue/discovery branch-point" inside this file — that mapping is real and still used, but it belongs entirely to a later, separate quest-design pass, not to this synthesis step. Keep this document's job narrow: follow each finding's implications as far as they can honestly go, and stop.
