...
| **4** | Mawson-Soyuz Highway | — | Sayowa → Mawson → Soyuz → *(ex-Bharati Japanese city, name TBD)* | **Corrected 2026-07-06:** substantially extended and reordered — previously just "Mawson → Bharati → Soyuz"; now runs the full Sayowa-to-Bharati stretch, with Soyuz and Bharati's order swapped. Sayowa end connects directly to Hwy 7-ext's endpoint there; the far end sits at a tri-junction connecting directly to both Hwy 110's and Hwy 22's endpoints. |
...
| **7-ext** | Belgrano Highway Extension | — | Lazar → Princess Elisabeth → *(junction with Hwy 37, shortly before Sayowa)* → Sayowa | **Corrected 2026-07-06:** added the junction with Hwy 37 shortly before reaching Sayowa. Sayowa terminus connects directly to Hwy 4's endpoint there. Built **2611–2614**; only highway with confirmed in-world construction dates. |
...
| **37** | Mountain Cut Throughway | — | *(junction with Hwy 7-ext, shortly before Sayowa)* → Dome Fuji → *(dual-junction with Hwy 22, bidirectional)* → Kunlun → Vostok → Concordia | **Corrected 2026-07-06 — this route was badly incomplete and out of order.** Dome Fuji is now a confirmed stop *(its own file's "no highway access" claim is stale — flagged for a fix)*. Kunlun and Vostok's order is reversed from the old listing. Concordia end connects directly to both Hwy 110's and Hwy 183's endpoints via the outer ring linking Concordia's Capricorn and Sagittarius districts. East Antarctic plateau traverse — not through the Transantarctic Mountains. |
