# Scott — Cultural Spec Sheet

---

**Subnet:** Janbogo
**Status:** Damaged; partially operational — Fort McMurdo's smaller, more intimate neighbor, three kilometers apart on the same peninsula
**Census II Population:** 153,382 humans / 160,252 robots / 313,634 combined *(rank 25th of ~32, corrected 2026-07-17 — was "24th," stale against the current Official_Population_Census.md Census II ranking table; "island cap," Ross Island's size limits growth for both Scott and Fort McMurdo)*
**Location:** Hut Point Peninsula, Ross Island (~77°51'S, 166°46'E), approximately 3km from Fort McMurdo

**One-liner:** *The city that knows exactly whose name it bears and why — New Zealand's founding culture reduced to Notable tier by Census II, yet Scott remains the most personally, ceremonially connected place in all of Tepenia to the man it's named for.*

---

## PART I — FOUNDATIONS

---

## 1. National/Ethnic Composition

| Tier | Nations |
|------|---------|
| Primary | China, USA |
| Significant | Japan, UK, South Korea, Canada, Indonesia, Australia *(founding wave)* |
| Notable | Philippines, Malaysia, New Zealand *(founding operator heritage)*, Chile |

*Scott carries the most extreme version of the founding-nation-persists pattern documented anywhere in the Janbogo subnet — arguably anywhere in Tepenia so far. New Zealand founded the city, named it, and set its earliest civic institutions in place, and by Census II sits at Notable tier — the lowest possible tier, below even Janbogo's Korea, Zukelli's Italy, or Dumont d'Urville's France, all of which at least held Significant tier. Yet Scott's early civic template — St. Robert's veneration, the precision-over-scale ethos, the whole character of the place — persisted anyway, independently carried forward by whoever actually lived there. The culture didn't just survive its founding nation's demographic collapse to Notable tier. It barely noticed.*

---

## 2. Founding Story

New Zealand exiles arrived to a station that had already been running, modestly and carefully, since 1957 — Scott Base was never built for scale the way McMurdo was; it was built for quality of relationship with a difficult environment, a design choice the founders carried straight into the exile city. The founders kept it modest on purpose, three kilometers from a city that would grow into the largest industrial center in the Federation, and never tried to compete with it on size.

The name was kept without hesitation: Robert Falcon Scott, who died returning from the Pole in 1912, had organized New Zealand's entire relationship with Antarctica for centuries before the exile ever happened. Keeping his name wasn't loyalty to Britain — New Zealand had been independent for centuries by then — it was inheritance of a specific Antarctic identity that predated the nation's independence and outlasted it.

---

## 3. Climate Character

Scott's climate is Fort McMurdo's climate, essentially without distinction — the same Ross Island geology, the same Mount Erebus dominating the skyline, the same ~118-day polar night, the same katabatic winds off the polar plateau. Three kilometers of shared rock means there was never room for the two cities' physical environments to diverge.

---

## 4. Seasonal Rhythms

Scott's seasonal rhythm runs identically to Fort McMurdo's — the same sea ice road opening and closing McMurdo Sound access, the same summer/winter inversion of overland and maritime connectivity. What differs is scale and pace: where Fort McMurdo's rhythms are organized around industrial rotation cycles, Scott's are organized around a smaller, more stable population whose relationship to the seasons is steadier, less driven by contract-labor churn.

---

## PART II — CULTURAL IDENTITY

---

## 5. Post-Culture Identity

**Name/concept:** *Whose Name It Bears*

Scott's civic identity rests on a single, thoroughly internalized fact: this city exists because of a man who died not knowing anyone would ever live here. Every other part of Scott's culture — its precision, its intimacy, its quiet competence — sits underneath that foundational relationship with St. Robert. Where Fort McMurdo defines itself by what it produces, Scott defines itself by what it remembers, and remembers more personally than any other city in Tepenia remembers its own Saint.

- **a. Founding tension** — The most extreme founding-tension gap documented in this project: New Zealand set Scott's earliest civic institutions in place (precision over scale, competence in difficult landscapes, and St. Robert's veneration all took root early) and now sits at Notable tier, the lowest possible ranking, beneath even Janbogo's or Zukelli's founding nations. The culture shows no sign of having noticed the demographic shift — St. Robert belongs to whoever lives in Scott now, regardless of where their own ancestry traces.
- **b. Fault lines** — The permanent, low-grade rivalry-and-partnership with Fort McMurdo: genuinely collaborative where it needs to be (shared logistics, shared emergency infrastructure, shared access roads), genuinely competitive in identity terms (Scott residents resent being read as "Fort McMurdo's little sibling"; Fort McMurdo residents carry a mild, good-natured condescension Scott residents find both understandable and irritating). Neither city can afford to let it become serious, because both need each other too much.

---

## 6. Social Contract & Unwritten Rules

- **a.** Quality and precision are valued over scale — a small, well-run operation is respected here in a way that would read as insufficient ambition at Fort McMurdo.
- **b. Hospitality character** — Warm, personal, small-city intimate — people know each other's names here in a way genuinely impossible at Fort McMurdo's scale.
- **c.** Steady and unhurried relative to Fort McMurdo's shift-driven pace — Scott's smaller, more stable population doesn't run on the same rotation-cycle urgency.
- **d. Public/private divide** — St. Robert's veneration and the Hut Point tradition are entirely, ceremonially public; the Fort McMurdo rivalry is discussed openly too, treated more as shared civic sport than private grievance.
- **e. Privacy as civic value** — Low — a small, stable, long-term-resident population naturally knows more about each other than a transient industrial city ever could, and is comfortable with that.

---

## 7. Who This City Attracts

- **The type this city draws:** people who want competence and intimacy over scale — long-term residents, precision-minded technical workers, anyone drawn to genuine reverence for the Antarctic exploration history Scott embodies more directly than anywhere else in Tepenia, and — per the developer's 2026-07-05 vision — families specifically, drawn by a quiet, residential, unhurried environment that Fort McMurdo's industrial scale simply cannot offer.
- **The type this city repels:** anyone chasing Fort McMurdo's scale of opportunity or anonymity — Scott offers neither.

---

## 8. Language

- **a. Primary language(s):** English (New Zealand-inflected) remains the civic default despite New Zealand's Notable-tier standing, alongside the Chinese and broader English of the China/USA Primary pairing.
- **b. Linguistic character** — Understated, precise, practical — a civic speech pattern that took root early and persists regardless of a given resident's own ancestry.
- **c. Slang / dialect markers** — References to "the Hut" and "St. Robert" function as immediately recognizable local shorthand, carrying real ceremonial weight rather than casual usage.
- **d. Language and integration** — Low pressure, small-city integration — a population this size and this stable absorbs newcomers through direct personal relationship rather than any formal linguistic expectation.

---

## 9. Architecture

Compact and carefully considered, deliberately modest by comparison to Fort McMurdo's sprawl — a founding-era design sensibility that valued a well-organized dozen structures over an ever-expanding industrial footprint. Hut Point itself, preserved rather than built around, anchors the city's whole relationship to its own history.

**Developer vision, established 2026-07-05 — a residential city, mostly:** in raw physical footprint, Scott is overwhelmingly residential — housing dominates the built environment, with the rest given over mainly to casual, leisurely business (restaurants, general leisure) and a modest handful of public gathering buildings. Walking through it, not much is visibly *happening* — no industrial hum, no constant traffic of decisions and dispatch the way Fort McMurdo has. This is not a contradiction of Scott's technical/research economic base (see Division of Industry below) so much as a distinction between where the work happens and what the city actually feels like to live in: the research and precision work is real, but it's contained, and the visible, lived texture of Scott is a genuinely decent, quiet place to raise a family.

---

## 10. Cuisine

- **a. Human cuisine** — Practical, shaped by a stable rather than transient population — home-style cooking more than canteen culture, a marker of Scott's smaller, more settled community.
- **b. Social context of eating** — Genuinely communal in the way a small, long-term-resident population's meals tend to be — closer to shared family cooking than Fort McMurdo's shift-refueling canteens.
- **c. Robot cuisine / siligel culture** — Modest and practical, without the industrial scale of Fort McMurdo's production, matching the city's overall precision-over-volume character.

---

## 11. Fashion

Practical, understated outdoor competence — clothing that reflects "comfortable with the outdoors" as a genuine cultural value rather than pure necessity, distinct from Fort McMurdo's purely functional workwear.

---

## 12. Music

Small, close-knit musical culture reflecting a stable long-term population — less about performance industry, more about the kind of music a community that actually knows each other shares informally.

---

## 13. Arts & Aesthetics

- **Visual arts:** Documentary and historical art centered on St. Robert, Hut Point, and the exploration-era heritage the city embodies more directly than any other Tepenian city.
- **Literature:** A strong tradition of historical and commemorative writing, tied closely to the Saints framework and the specific Tepenian reframing of Scott's death as purposeful sacrifice rather than tragedy.
- **Crafts & material culture:** Precise, considered craftsmanship valued for its quality over its scale, consistent with the whole city's founding ethos.

---

## 14. Sensory Profile

- **Sound:** Quieter than Fort McMurdo — the same katabatic winds and occasional Erebus rumble, but without the constant industrial machinery hum.
- **Smell:** Cleaner and less industrial than Fort McMurdo, though still carrying the shared volcanic and cold-climate baseline of Ross Island.
- **Feel:** Intimate and precise — a city small enough to feel navigable and known in a way Fort McMurdo never could.
- **First impressions for visitors:** The scale contrast with Fort McMurdo registers almost immediately for anyone arriving from there — Scott feels deliberately smaller, quieter, more considered.

---

## PART III — SOCIAL STRUCTURE

---

## 15. Division of Industry

- Technical / scientific: 30% — precision and research-oriented work, distinct from Fort McMurdo's extraction focus
- Commercial: 20% — port and shipping logistics (New Zealand freighter trade, per the established Ross Sea supply route)
- Education: 15% — historical/commemorative knowledge-keeping tied to St. Robert
- Industrial / manufacturing: 15% — **confirmed 2026-07-07:** Scott's one genuine industry beyond its residential/political character is collecting volcanic material fallen from Mount Erebus and delivering it to a trucking facility across McMurdo Sound, forwarded onward down the coastal highway into the Janbogo subnet for further processing — a small, contained operation, consistent with the city's own quiet civic identity
- Marine / resource extraction: 12%
- Other: 8%

---

## 16. Human-Robot Relations

Close to parity (160,252 robots to 153,382 humans) in a small, stable, long-term community where human-robot relations are shaped by genuine personal familiarity rather than the more impersonal, role-driven dynamics of a larger industrial city like Fort McMurdo — people, robot and human alike, simply know each other here.

---

## 17. Robot-Specific Culture

Robot culture at Scott benefits from the same small-community intimacy that shapes everything else — robots are woven into the city's long-term social fabric in a way that's harder to achieve at Fort McMurdo's transient scale, and many are directly involved in the historical/commemorative work surrounding St. Robert and Hut Point.

---

## 18. Religious / Philosophical Landscape

- St. Robert's veneration is the single most significant belief-adjacent practice in Scott, and the most personally felt Saint relationship anywhere in Tepenia — not abstract civic theology but a lived, daily awareness that the city exists because of one specific man's unknowing sacrifice.
- The Tepenian reframing of Scott's 1912 death — not tragic failure, but purposeful (if unknowing) gift to a people who didn't yet exist — is held here with more direct personal weight than anywhere else it's honored.

---

## 19. Underground / Counterculture

Minimal — a small, stable, deeply commemorative community doesn't generate much appetite for counterculture against its own dominant civic identity; what exists is closer to private skepticism about the Fort McMurdo rivalry's intensity than any organized subculture.

---

## 20. Private Life & Minority Communities

The Significant and Notable tier nations (including New Zealand itself, now reduced to Notable tier) maintain identity through personal relationship in a community small enough that private and public life blur more than in larger cities — everyone's background is generally known, without it becoming the city's organizing civic fact.

---

## PART IV — POLITICAL & EXTERNAL

---

## 21. Political Character

Quiet and locally focused, its political energy directed mostly toward maintaining independence of identity from Fort McMurdo rather than any Federation-wide projection — a small city's politics are mostly about not being absorbed, culturally, by its much larger neighbor.

---

## 22. Relationship to Upper Earth

Filtered heavily through the Saints framework rather than raw anger or grief — Scott's relationship to Upper Earth is inseparable from its relationship to St. Robert, a pre-War figure whose sacrifice is honored specifically because it happened before any of the anti-robot conflict existed. This gives Scott a uniquely reverent, rather than embittered, lens on its own pre-exile history.

---

## 23. Relationship to Other Cities

- **Fort McMurdo:** The defining relationship of Scott's entire existence — collaborative on logistics and infrastructure, competitive on identity, permanently intertwined by three kilometers of shared rock and shared history. Neither city can fully separate its own story from the other's.
- **Janbogo:** Connected via Hwy 183 north; part of the same Ross Sea coastal network.
- **Framheim / Little America** *(removed from canon 2026-07-03)*: Were fellow Ross Ice Shelf-region sites with their own pre-exile expedition heritage, sharing the same Saints-framework civic reverence Scott holds for St. Robert. Both cities were permanently removed from Tepenian canon — their shared real-world site, the Bay of Whales, was eliminated by the 1987 Iceberg B-9 calving event, leaving no physical basis for either. Scott no longer has a living relationship with either city.

---

## 24. Arcanet Culture

Closely linked with Fort McMurdo's network infrastructure given the physical proximity, but carrying more commemorative and historical archive traffic proportionally — St. Robert and Hut Point material feature heavily in Scott's own local Arcanet use.

---

## 25. Export Culture

- Precision technical expertise — Scott-trained workers recognized for quality-over-scale competence, a direct contrast to Fort McMurdo's raw industrial output
- Historical/commemorative knowledge — the city's deep, specific relationship with the Saints framework and pre-exile exploration history
- A specific *type of person* — quietly competent, historically minded, comfortable being the smaller of two neighboring cities without needing to compete on Fort McMurdo's terms

---

## 26. Municipal Holidays

- **Tepenian Independence Day (June 21) — Hut Point Remembrance** — the city's central civic observance: residents of both Scott and Fort McMurdo gather at Hut Point to lay candles, flowers, and personal tokens in memory of St. Robert. Not mandated, never has been — it simply happens, year after year, because both cities feel it should. The Saint belongs to the whole peninsula, not only to the city that bears his name.

---

## PART V — VISITOR & DIASPORA

---

## 27. Visitor Experience

The contrast with Fort McMurdo registers first for most visitors, who arrive via the same spur road and same access point — Scott feels immediately smaller, quieter, more considered. Hut Point itself carries a specific ceremonial weight that visitors notice even without knowing the full history behind it.

---

## 28. The Visitor-to-Resident Transition

Becoming a local at Scott means genuinely internalizing St. Robert's story and the Hut Point tradition as your own — in a city this small and this historically self-aware, that shift happens through direct personal relationship and participation in the remembrance tradition rather than any formal integration process.

---

## PART VI — REFERENCE

---

## 29. Notable Local Landmarks

- **Hut Point** — Scott's most sacred civic location; Robert Falcon Scott's 1902 discovery hut, among the oldest surviving human-made structures in Antarctica; site of the annual Tepenian Independence Day remembrance shared by both Scott and Fort McMurdo
- **The Scott–Fort McMurdo border** — a formally maintained but practically porous line between the two cities
- **Mount Erebus** — shared skyline presence with Fort McMurdo, equally dominant from Scott

---

## 30. Significant Local Events

- **The Long Night War damage** — Scott was damaged alongside Fort McMurdo, their intertwined infrastructure meaning what hit one likely affected the other; exact coordination or separation during the war itself is TBD

---

## 31. Notable Figures

- **Community Organizer Fiona Māui** *(placeholder, added 2026-07-17, proposed `Scott_Full_Extrapolation.md` §V)* — credited with shaping Scott's domestic, family-oriented civic character in its early exile-era decades.
- **Keeper of Hut Point Wiremu Tane** *(placeholder, added 2026-07-17, same section)* — a ceremonial figure associated with maintaining the Independence Day remembrance tradition at Hut Point.

---

## 32. Diaspora Character in Concordia

Scott refugees in Concordia disperse rather than concentrate, given the city's small population — where known, they tend to be identified through the Fort McMurdo association ("from Ross Island") more than as a distinct Scott community in their own right. What marks them individually is a quiet, precise competence and an unusually direct, personal relationship to the Saints framework — Scott-descended Concordians are more likely than most to actually know the details of St. Robert's story rather than treating Tepenian sainthood as distant civic tradition.
