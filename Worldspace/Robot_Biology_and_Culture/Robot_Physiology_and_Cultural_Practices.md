...
- **Robot creation mechanism — "fabrication-synthesis chambers" (placeholder name)** — established 2026-07-06, during Soyuz's City Vision Notes session (see `Cities/Local_Cultures/Mirny_Subnet/Soyuz.md` and `Cities/Specs/Soyuz.md`): robots are physically created via an apparatus manufactured in only a handful of Tepenian cities (Soyuz confirmed as one). These chambers are shipped nationwide from their manufacturing sites, meaning a robot can be built ("born") in any Tepenian city regardless of whether that city manufactures chambers itself.
