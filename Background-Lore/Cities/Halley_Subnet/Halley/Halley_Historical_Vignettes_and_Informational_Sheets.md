# Halley — Historical Vignettes & Informational Sheets
...
Halley's entire architecture is built around ski-mounted, actively-propelled platform structures — a
founding-era design choice to build for relocation rather than resist the Brunt Ice Shelf's own seaward drift
(Physical Infrastructure Attribute #1). The first module ever moved under its own propulsion did so before
the system had ever been tested at full scale, on a shelf whose drift behavior the founding engineers were
still learning to predict.

The maiden propulsion run covered a distance later generations would consider trivial — a few hundred meters,
completed over the better part of a day rather than the hours it would take a fully-calibrated module decades
later. The engineers responsible logged every measurable variable they could think to check: track tension,
ski-surface friction against the specific ice conditions of that week, structural flex under sustained
low-speed load. None of it had a precedent to compare against. The module reached its target position intact,
and the dedicated engineering discipline that now maintains every ski-mounted structure in the city
...
The Position Archive (attribute #8) is, per Halley's own established lore, possibly the only surviving trace
of the city's full geographic history — since old sites aren't merely abandoned the way ruins are elsewhere in
Tepenia, but lost outright to the calving edge and the sea beyond it (Finding A, Finding B). Whoever maintains
that archive is maintaining the only copy of something that cannot be re-derived by simply walking out and
looking.

**"Perpetua Adair" *(placeholder, not developer-chosen)*** has kept the Position Archive for several decades,
long enough that she has personally overseen the transfer of records from at least two Old Positions before
those specific sites became inaccessible to the calving edge. She has never seen Halley's actual founding-era
position — it was lost before her own tenure began, per Finding B's own established limit on which Old
Positions remain physically accessible at all — and by her own account, the founding position exists for her
entirely as paper and survey data, the same secondhand relationship every resident born after its loss now
has to it. She has, however, personally walked every Old Position still standing, and keeps a private,
